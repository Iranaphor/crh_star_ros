class ROSSim():
    ...
    def Launch_Agent(self, node_id, heuristic_details):
        ...
        A.agent_id = length(self.Agent_List)+1
    ...
    def TimeStep(self, t):
        ...
        for EExt = self.Coordinator.Overlay.EdgeExtensions:
            EExt.PurgeOld(t)


        #update agent locations
        for A = self.Agent_List:
    ...
    def getMeaningfulNode(self, A, Config):
        ...
        rowNodes = cell2mat([Config.meta.rowNodes_start, Config.meta.rowNodes_end])'
        cropNodes = []
        if Config.meta.bad_order:
            cropNodes = cell2mat(Config.meta.cropNodes)
        else
            for row=rowNodes':
                cropNodes = [cropNodes, row(1):row(2)]
        ...
        if A.agentType == "logistics":
            ...
            MAXNode = length(self.Coordinator.Network.node_list)
    ...
    def Show_Agents(self):
        self.Coordinator.Network.plot
        self.Coordinator.Network.plotNodeNames
        for agent = self.Agent_List:
            agent.currentNode.ChangeState("selected")
            pos = agent.currentNode.position
            text(pos(1)-.5, pos(2)+.5, string(agent.agent_id), "Color", [1,0,1])
    ...
    def Show_Movement(self, Reserv_List, time_in, time_out):
        ...
        #Plot connections
        for aid = [self.Agent_List.agent_id]:
    ...
    def Show_AgentMovement(self, R, t1, ~):
